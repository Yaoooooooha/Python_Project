...
def process():
    ...
    if not os.path.isdir(Class_File_Dir_Path):
        ...
        msg.showinfo(title='資料夾已建立!', message='課程資料夾已建立於C槽(C:)，\n請在資料夾中新增課程檔案')
        os._exit(0)
    # 檢查有沒有專班課程時間檔案
    ...
    for record_attendance in record_attendance_file:
        ...
        num = xlsx_file[-7]
        # 試讀出席檔案
        try_record_file = pd.read_csv('./課程資料/' + csv_file, encoding='utf-16', sep='\t', on_bad_lines='skip')
        row_index = 0
        while True:
            ...
            if '會議' in row:
                break
            row_index += 1
        # 確認csv檔的姓名標頭為“顯示名稱”或是“姓名”
        h = 0
        try:
            record_file = pd.read_csv('./課程資料/' + csv_file, encoding='utf-16', header=row_index, sep='\t',
                                      usecols=['顯示名稱', '出席者電子郵件', '加入時間', '離開時間', '出席時間'])
        except:
            h += 1
        try:
            record_file = pd.read_csv('./課程資料/' + csv_file, encoding='utf-16', header=row_index, sep='\t',
                                      usecols=['姓名', '出席者電子郵件', '加入時間', '離開時間', '出席時間'])
        except:
            h -= 1
        # 讀檔案
        if h > 0:
            ...
            record_file = pd.read_csv('./課程資料/' + csv_file, encoding='utf-16', header=row_index, sep='\t',
                                      usecols=['顯示名稱', '出席者電子郵件', '加入時間', '離開時間', '出席時間'])
            names = record_file['顯示名稱']
        else:
            print("194行出問題了")
        attendance_file = open_excel('./課程資料/' + xlsx_file)

        in_times = []
        for the_time in record_file['加入時間']:
            the_time = the_time[2:-1]
            if the_time == '[]':
                continue
            in_times.append(the_time)
        out_times = []
        for the_time in record_file['離開時間']:
            the_time = the_time[2:-1]
            if the_time == '[]':
                continue
            out_times.append(the_time)

        # 統計上線時間
        name_online_dict = dict()
        for index, name in enumerate(names):
            # 忽略空值
            if str(in_times[index]) == 'nan' or str(out_times[index]) == 'nan' or in_times[index] == None or out_times[index] == None:
                continue
            # 設定時間格式
            time_format = '%Y-%m-%d %H:%M:%S'
            in_time = dt.datetime.strptime(in_times[index], time_format).strftime('%H:%M')
            out_time = dt.datetime.strptime(out_times[index], time_format).strftime('%H:%M')
        ...
        # 設立標準時間
        std_time = dt.datetime.strptime(in_times[0], time_format)
        year = std_time.year
        mon = std_time.month
        day = std_time.day
        ...
        if spe_class and spe_class_file_exist:
            ...
            for index_spe, class_name in enumerate(spe_class_file[spe_name_col]):
                if spe_name == class_name:
                    ...
                    class_start = dt.datetime(year, mon, day, int(start_end[0][:2]), int(start_end[0][2:])).strftime(
                        '%H:%M')
                    class_end = dt.datetime(year, mon, day, int(start_end[1][:2]), int(start_end[1][2:])).strftime(
                        '%H:%M')
